  ...
  try {
    ...
    xml2js.parseString(xmlText, { explicitArray: false }, async (err, result) => {
        ...
        eventsArray = Array.from(items).map(item => ({
          ...
          title: item.titlee,
          date: item.predateE,
          duration: item.progtimee!=""?item.progtimee:'Not Applicable',
          venue: item.venueid,
          descre: item.desce!=""?item.desce:'Not Applicable',
          pre: item.presenterorge!=""?item.presenterorge:'Not Applicable'
        }));
...
//random select 10 venue from the venue api
venueShown = venueShown.sort(()=> 0.5 - Math.random()).slice(0,10);
console.log(venueShown.length);
...
eventsArray = eventsArray.sort(()=> 0.5 - Math.random()).slice(0,200);
...
for(let i=0;i<eventsArray.length;i++){
  let newEvent = new Event({
    ...
    venue: eventsArray[i].venue,
    date: eventsArray[i].date,
    duration: eventsArray[i].duration,
